...
from univlg.modeling.backproject.backproject import interpolate_feats_3d
...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    ...
    def forward(self, srcs, pos_embeds):
        (
            ...
            level_start_index,
            valid_ratios,
        ) = self.prepare_inputs(srcs, pos_embeds)
...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            self.cfg.MODEL.DECODER_3D
            and self.cfg.MODEL.CROSS_VIEW_CONTEXTUALIZE
            and not self.cfg.MODEL.NO_DECODER_PANET
        ):
            self.cross_view_attn = nn.ModuleList(
                [
                    CrossViewPAnet(
                        latent_dim=conv_dim,
                        nsample=self.cfg.MODEL.KNN,
        ...
        self.num_fpn_levels = 1 if (self.cfg.FORCE_VIT_XYZ_SCALE or self.cfg.FORCE_SINGLE_FPN_LEVEL) else int(np.log2(stride) - np.log2(self.common_stride))
        ...
        for idx, in_channels in enumerate(self.feature_channels[: self.num_fpn_levels]):
            ...
            if (
                self.cfg.MODEL.DECODER_3D and self.cfg.DO_TRILINEAR_INTERPOLATION
            ) or self.cfg.USE_CONV1D:
                output_conv = Conv2d(
                    conv_dim,
                    conv_dim,
                    kernel_size=1,
    ...
    def forward_features(
        ...
        if not self.training and (
            (self.cfg.INPUT.MIN_SIZE_TEST > 500 and srcs[0].shape[0] > 100)
            or srcs[0].shape[0] > 150
        ):
            max_bs = 100
            ...
            for i in range(num_splits):
                srcs_ = [
                    srcs[j][i * max_bs : (i + 1) * max_bs] for j in range(len(srcs))
                ]
        ...
        if decoder_3d and self.cfg.MODEL.CROSS_VIEW_CONTEXTUALIZE:
            out_new = []
            for i in range(self.transformer_num_feature_levels):
                mv_data = {}
                ...
                out_new.append(
                    self.cross_view_attn[i](
                        feature_list=[out[i]],
                        xyz_list=[x_xyz[1 if self.cfg.FORCE_VIT_XYZ_SCALE else i]],
                        shape=shape[:2],
            ...
            for idx, f in enumerate(self.in_features[: self.num_fpn_levels][::-1]):
                ...
                if (decoder_3d and self.cfg.DO_TRILINEAR_INTERPOLATION):
                    ...
                    shape_ = (
                        ...
                        and self.training
                        and self.cfg.MODEL.MASK_FORMER.TRAIN_NUM_POINTS != -1
                        and self.cfg.HIGH_RES_SUBSAMPLE
                        else shape[:2]
                    )
                    interp = interpolate_feats_3d(
                        out[-1],
                        x_xyz[len(out) - 1],
                        multi_scale_p2v[len(out) - 1]
